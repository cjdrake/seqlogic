...
class FbebCtl(Module):
    ...
    def build(self):
        # Ports
        rd_addr = self.output(name="rd_addr", dtype=Array[1])
        wr_addr = self.output(name="wr_addr", dtype=Array[1])

        empty = self.output(name="empty", dtype=Array[1])
        full = self.output(name="full", dtype=Array[1])
